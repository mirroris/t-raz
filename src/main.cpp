...
private:
    ...
    static VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
        ...
        void* pUserData) {
            std::cerr << "validtion layer: " << pCallbackData->pMessage << std::endl;
            return VK_FALSE;
    }
    ...
    void createCommandPool() {
        ...
        if(vkCreateCommandPool(logicalDevice_, &createInfo, nullptr, &commandPool_) != VK_SUCCESS){
    ...
    void createInstance() {
        ...
        if (enableValidationLayers && !checkValidationLayerSupport()) {
            throw std::runtime_error("validation layers requested, but not avaialble!");
        }
